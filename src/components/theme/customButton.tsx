...
const CustomButton: React.FC<IProps> = ({
  ...
  return (
    <button
      type="button"
      {...rest}
      className={`relative shrink-0 border-[1.5px] font-semibold min-w-[90px] inline-flex items-center justify-center ${variantColor === "primary" && variantType === "outlined"
    ...
    >
      ...
      <span
        className={`absolute translate-x-[-50%] translate-y-[-50%] top-[50%] left-[50%] ${!loading ? "opacity-0 invisible" : ""
          }`}
      >
        <PulseLoader color="white" size={10} />
      </span>
      <span
        className={`flex items-center gap-[8px] ${loading ? "opacity-0 invisible" : ""
          }`}
      >
        {leftIcon}
        {rest.children}
