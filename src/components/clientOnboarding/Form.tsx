...
import { getSubscriptionTypeDependentStepName } from "@/components/clientOnboarding/clientSubscriptionSteps.tsx/step-1/SubscriptionType";
...
const Form = () => {
        ...
        if (res.status) {
          ...
          if (step1Data) {
            if (step1Data.investor_type)
              stepperState.setData(0, 1, {
                investor_type: step1Data.investor_type,
              });
            if (step1Data.subscription_amount)
              stepperState.setData(0, 2, {
                subscription_amount: step1Data.subscription_amount,
              });
            if (step1Data.u_s_personhood)
              stepperState.setData(0, 3, {
                u_s_personhood: step1Data?.u_s_personhood,
              });
            if (
              step1Data.subscriber_first_name &&
              step1Data.subscriber_last_name
            )
              stepperState.setData(0, 4, {
                subscriber_first_name: step1Data.subscriber_first_name,
                subscriber_last_name: step1Data.subscriber_last_name,
              });
            if (step1Data.subscription_type)
              stepperState.setData(0, 5, {
                subscription_type: step1Data.subscription_type,
              });
            if (
              ...
              stepperState.setData(0, 7, {
                address_line_1: step1Data.address_line_1,
                address_line_2: step1Data.address_line_2,
                city: step1Data.city,
                state: step1Data.state,
                zip_code: step1Data.zip_code,
                country: step1Data.country,
              });
            if (step1Data.dob && step1Data.pob_country)
              stepperState.setData(0, 8, {
                dob: format(new Date(step1Data?.dob), "MM/dd/yyyy"),
                pob_country: step1Data.pob_country,
              });
            if (step1Data.citizenship_country)
              stepperState.setData(0, 9, {
                citizenship_country: step1Data?.citizenship_country,
              });
            if (step1Data.ssn_tin)
              stepperState.setData(0, 10, { ssn_tin: step1Data.ssn_tin });
            if (
              step1Data.subscriber_phone_no ||
              step1Data.subscriber_phone_code
            ) {
              stepperState.setData(0, 11, {
                subscriber_phone_code: step1Data.subscriber_phone_code,
                subscriber_phone_no: step1Data.subscriber_phone_no,
              });
            ...
            if (step1Data.joint_ownership_type) {
              stepperState.setData(0, 6, {
                joint_ownership_type: step1Data?.joint_ownership_type,
              });
            }
            if (step1Data.benefit_plan_type) {
              stepperState.setData(0, 6, {
                benefit_plan_type: step1Data?.benefit_plan_type,
              });
            }
            if (step1Data.subscription_process_notice) {
              stepperState.setData(0, 6, {
                subscription_process_notice:
                  step1Data.subscription_process_notice,
              });
            }
            if (
              step1Data.subscription_type &&
              step1Data.subscription_type != "4"
            ) {
              stepperState.updateSubStep(0, 6, {
                skip: false,
                name: getSubscriptionTypeDependentStepName(
                  step1Data.subscription_type.toString()
                ),
              });
            }
          }
          ...
          if (step2Data) {
            stepperState.setData(1, 0, {
              accreditation_status: step2Data.accreditation_status,
            });
            if (step2Data.qualified_client_status)
              stepperState.setData(1, 1, {
                qualified_client_status: step2Data.qualified_client_status,
              });
            if (step2Data.qualified_purchaser_status)
              stepperState.setData(1, 2, {
                qualified_purchaser_status:
                  step2Data.qualified_purchaser_status,
              });
            if (step2Data.placement_agent_check)
              stepperState.setData(1, 3, {
                placement_agent_check: step2Data.placement_agent_check,
              });
            if (step2Data.placement_agent_name)
              stepperState.setData(1, 4, {
                placement_agent_name: step2Data.placement_agent_name,
              });
            if (step2Data.investor_occupation)
              stepperState.setData(1, 5, {
                investor_occupation: step2Data?.investor_occupation,
              });
            if (step2Data.is_relative_investment)
              stepperState.setData(1, 6, {
                is_relative_investment: step2Data?.is_relative_investment,
              });
            if (step2Data.is_risk_consent)
              stepperState.setData(1, 7, {
                is_risk_consent: step2Data?.is_risk_consent,
              });
            if (step2Data.is_prior_fund_investment)
              stepperState.setData(1, 8, {
                is_prior_fund_investment: step2Data?.is_prior_fund_investment,
              });
            if (step2Data.bad_actor_check)
              stepperState.setData(1, 9, {
                bad_actor_check: step2Data?.bad_actor_check,
              });
            if (step2Data.political_exposed_person)
              stepperState.setData(1, 10, {
                political_exposed_person: step2Data?.political_exposed_person,
              });
            if (step2Data.political_exposure_context)
              stepperState.setData(1, 11, {
                political_exposure_context:
                  step2Data?.political_exposure_context,
              });
            if (step2Data.political_exposure_role)
              stepperState.setData(1, 12, {
                political_exposure_role: step2Data?.political_exposure_role,
              });
  ...
  return (
    <div
      className={`w-100 min-h-[100vh] bg-danger position-relative ${styles.subscriptionLayout}`}
    >
      <div className={``}>
        <InvestorSubscriptionSidebar
          toggleSidebar={() => {}}
          isSidebarOpen={true}
        />
      </div>
